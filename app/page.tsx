...
import { Search, FolderOpen, ShieldCheck, Sparkles, Image as ImageIcon } from 'lucide-react';
  ...
  return (
    <>
      ...
      <motion.section
        ...
        className="relative overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-800 bg-gradient-to-r from-emerald-500 via-emerald-600 to-teal-600 text-white"
      >
      ...
      {/* Gallery (2 foto) — versi modern tanpa komponen tambahan */}
      <motion.section
        initial={{ opacity: 0, y: 8 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, amount: 0.2 }}
...
export default function Page() {
  ...
  function handleOpen(item: LinkItem) {
    if (!item.href) return;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 text-slate-800 dark:from-slate-900 dark:to-slate-950 dark:text-slate-100">
      {/* Header (logo PTPN 4 Reg III sudah di AppHeader) */}
      <AppHeader />

      {/* BODY */}
      <div className="mx-auto max-w-[1400px] px-2 sm:px-4 py-4 grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4">
        {/* SIDEBAR */}
        <Sidebar activeKey={activeKey} onSelect={(k) => { setActiveKey(k); setSearch(''); }} />
        ...
        <main className="space-y-4">
          {activeKey === 'home' ? (
            ...
            <section className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 p-5">
              ...
              <ContentGrid items={filtered} onOpen={handleOpen} />
            </section>
          )}
        </main>
      </div>
